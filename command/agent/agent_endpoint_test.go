...
func makeReadOnlyAgentACL(t *testing.T, srv *HTTPServer) string {
	...
	req, _ := http.NewRequest("PUT", "/v1/acl/create?token=root", tojson(args))
...
func TestAgent_RegisterCheck(t *testing.T) {
	...
	req, _ := http.NewRequest("GET", "/v1/agent/check/register?token=abc123", tojson(args))
...
func TestAgent_RegisterCheck_Passing(t *testing.T) {
	...
	req, _ := http.NewRequest("GET", "/v1/agent/check/register", tojson(args))
	obj, err := srv.AgentRegisterCheck(nil, req)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
...
func TestAgent_RegisterCheck_ACLDeny(t *testing.T) {
	...
	// Try the root token.
	req, _ = http.NewRequest("GET", "/v1/agent/check/register?token=root", tojson(args))
...
func TestAgent_UpdateCheck(t *testing.T) {
	...
	for _, c := range cases {
		req, _ := http.NewRequest("PUT", "/v1/agent/check/update/test", tojson(c))
		resp := httptest.NewRecorder()
	...
	// Make sure abusive levels of output are capped.
	{
		...
		req, _ := http.NewRequest("PUT", "/v1/agent/check/update/test", tojson(args))
		resp := httptest.NewRecorder()
		obj, err := srv.AgentCheckUpdate(resp, req)
		if err != nil {
			t.Fatalf("err: %v", err)
	...
	// Check a bogus verb.
	{
		...
		req, _ := http.NewRequest("POST", "/v1/agent/check/update/test", tojson(args))
		resp := httptest.NewRecorder()
		obj, err := srv.AgentCheckUpdate(resp, req)
		if err != nil {
			t.Fatalf("err: %v", err)
...
func TestAgent_UpdateCheck_ACLDeny(t *testing.T) {
	...
	// Try with the root token.
	args = checkUpdate{api.HealthPassing, "hello-passing"}
	req, _ = http.NewRequest("PUT", "/v1/agent/check/update/test?token=root", tojson(args))
...
func TestAgent_RegisterService(t *testing.T) {
	...
	req, _ := http.NewRequest("GET", "/v1/agent/service/register?token=abc123", tojson(args))
...
func TestAgent_RegisterService_ACLDeny(t *testing.T) {
	...
	// Try with no token.
	req, _ := http.NewRequest("GET", "/v1/agent/service/register", tojson(args))
	if _, err := srv.AgentRegisterService(nil, req); !isPermissionDenied(err) {
		t.Fatalf("err: %v", err)
	}

	// Try with the root token.
	req, _ = http.NewRequest("GET", "/v1/agent/service/register?token=root", tojson(args))
...
func TestAgent_RegisterCheck_Service(t *testing.T) {
	...
	req, _ = http.NewRequest("GET", "/v1/agent/check/register", tojson(checkArgs))
	if _, err := srv.AgentRegisterCheck(nil, req); err != nil {
		t.Fatalf("err: %v", err)
	}
